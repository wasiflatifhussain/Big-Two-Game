/**
 * This hand consists of five cards with consecutive ranks and the same suit. For the sake of simplicity, 2 and A can only form a straight flush <br>
 * with K but not with 3. The card with the highest rank in a straight flush is referred to as the top card of this straight flush. A straight flush <br>
 * always beats any straights, flushes, full houses, and quads. A straight flush having a top card with a higher rank beats a straight flush having a <br>
 * top card with a lower rank. For straight flushes having top cards with the same rank, the one having a top card with a higher suit beats the one <br>
...
public class StraightFlush extends Hand{
	...
	/**
	 * The constructor to form a straight flush in this iteration of the game.
	 * @param player The player who played the hand of cards.
	 * @param cards The hand of cards played by the player.
	 */
	...
	/**
	 * Checks if the hand is actually a valid straight-flush or not.
	 * @return True if it is a valid straight-flush; false otherwise.
	 */
	public boolean isValid() {
		if (this.size() != 5) {   //if number of cards is not 5, it must not be a straight flush
			return false;
		}
		else if (this.size() == 5) {
			...
			for (int i = 0; i < 5; ++i) {   //make new tempArrayay and take the checkable index values for ranks
				tempArray[i] = switchChecker(this.getCard(i).rank);
			}
	        ...
	        for (int k = 0; k < tempArray.length; ++k) {  
	            ...
	            for (int j = k+1; j < tempArray.length; ++j) {     
	               if(tempArray[k] > tempArray[j]) {    
	                   temp = tempArray[k];    
	                   tempArray[k] = tempArray[j];    //swapping the elements in array for sorting 
	                   tempArray[j] = temp;    
	               }     
	            }     
	        }
			...
			//check if all cards have same suit or not
			for (int i = 1; i < 5; ++i) {
